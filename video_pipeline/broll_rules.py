...
def enforce_broll_schedule_rules(
    ...
    *,
    ...
    - Rejects clips that begin before ``min_start_s`` (hook preservation)
    - Enforces a minimum gap ``min_gap_s`` between consecutive clips
    - Blocks repeated assets for ``no_repeat_s`` seconds unless we still need
      to reach the target volume (``target_total``)
    - Warns when the spacing exceeds ``max_gap_s`` if provided
    """
    if not clips:
        return []
